...
class ACDASlideButtonWD extends StatefulWidget {
  const ACDASlideButtonWD({
    required this.backgroundColor,
    required this.slidingBarColor,
    required this.onButtonSlide,
    required this.onButtonOpened,
    required this.onButtonClosed,
    Key? key,
    this.slidingChild,
  ...
  @override
  State<ACDASlideButtonWD> createState() => _ACDASlideButtonWDState();
}

class _ACDASlideButtonWDState extends State<ACDASlideButtonWD> with SingleTickerProviderStateMixin {
  ...
  void initState() {
    ...
    _slideAC = AnimationController(vsync: this, duration: const Duration(milliseconds: 300))
      ..addListener(() {
        ...
        widget.onButtonSlide(_slideAC.value);

        if (_slideAC.value == 1.0) widget.onButtonOpened();

        if (_slideAC.value == widget.initialSliderPercentage) widget.onButtonClosed();
      });
          ...
          Align(
            ...
            child: GestureDetector(
              ...
              child: SizedBox(
                ...
                child: Align(
                  alignment:
                      widget.slideDirection == SlideDirection.right ? Alignment.centerLeft : Alignment.centerRight,
